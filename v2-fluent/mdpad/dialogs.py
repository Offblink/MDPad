...
### 版本历史
- **v1.5.2（当前版，v2-fluent 目录）**：修复输入法候选框跑到窗口左上角
...
### 本次更新（v1.5.2）
- **修复中文输入法候选框位置**：自绘光标把原生光标宽度设成了 0，Qt 给输入法的光标矩形查询（`ImCursorRectangle`）于是返回**宽度 0** 的矩形（实测 `QRectF(x, y, 0, h)`，原生是 1），Windows 拿它定位候选框/组字框就会退化成"窗口左上角"。现在重写 `inputMethodQuery`，把该矩形补成正常宽度（位置仍是当前光标位置）
