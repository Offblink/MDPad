...
### 版本历史
- **v1.5.1（当前版，v2-fluent 目录）**：修复退格/删除时光标滑行动画起点算错
...
### 本次更新（v1.5.1）
- **修复退格 / 删除时光标动画起跳**：v1.5.0 用"上一次的光标位置"在新文本里重新算动画起点，而退格是"先删字、再移动光标"，那个旧位置往往已经落到别处（删掉换行就变成另一行，删到文档末尾甚至算到布局之外），于是光标会从很远的行上飞过来。现在起点固定取"改文档之前屏幕上真正画着的那个矩形"，终点每次按当前光标位置现算（这样滚动、重排之后终点也自动是对的）
- **修复退格 / 删除时光标抖动**：编辑类（打字 / 退格 / 删除）与导航类（方向键 / 点击 / 查找跳转）分两套行为
  - **编辑**：光标紧贴文字，只做 40ms 的短促跟手滑行；连打（间隔 < 90ms）、删换行、删选区、整段重排一律直接落位；不改色、不留尾迹、不做脉冲。实测连打退格 8 次：画面光标与真实位置偏差 0–4px、无向右回退帧
  - **导航**：完整滑行（L 形路径：先纵向到位再横向）+ 运动期间强调色 + 80ms 渐隐尾迹 + 落点脉冲；落点在视口外时先把滚动条平滑滚过去
