...
- **v1.5.4**：输入法候选框定位的真正修法——平时保留原生光标（静止/组词/打字全走 Qt 原生路径），只在滑行动画那 ~100ms 临时关掉它由自绘接管；`setCursorWidth(0)` 长期关闭会把 `ImCursorRectangle` 变成宽度 0、Windows 层 `hwndCaret=None`，输入法据此定位就会贴到角落/上一行
