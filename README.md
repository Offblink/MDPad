...
- 光标平滑移动：滑行 + 渐隐尾迹 + 落点脉冲，运动期间用强调色（输入法组词时不动画）
...
- **v1.5.1**：修复退格 / 删除时光标的两处问题——① 动画起点算错（v1.5.0 拿"上一次的光标位置"在新文本里重算起点，删字后那个位置常已落到别的行、甚至文档末尾之外，光标会从很远的地方飞过来）；② 抖动（编辑类与导航类分成两套：打字/退格紧贴文字、只做 40ms 短促滑行，连打与删换行直接落位且不改色不留尾迹不脉冲；方向键/点击/查找跳转仍是完整滑行 + 强调色 + 尾迹 + 落点脉冲）
